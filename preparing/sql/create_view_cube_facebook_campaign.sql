...
-- view
--
create or replace view {SCHEMA}cube_facebook_campaign as
with date_range as (
...
select app_display_name as product_name, campaign_id, decode(product_platform, 'web', product_platform, platform) as platform, date_start as effective_date,
max(campaign_name) as campaign_name,
SUM(spend) as spend, SUM(impressions) AS impressions, SUM(clicks) AS clicks, min(crystallized::int)::bool as crystallized
...
group by app_display_name, campaign_id, decode(product_platform, 'web', product_platform, platform), effective_date