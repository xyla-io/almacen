...
-- view
--
create or replace view {SCHEMA}cube_by_campaign as 
  select effective_date, max(product_id) as product_id, max(product_name) as product_name,
  channel, mmp, campaign_id, platform, event_name,
  ...
  group by effective_date, channel, mmp, campaign_id, platform, event_name,
  daily_cohort, weekly_cohort, event_day, event_week, event_month