...
-- view
--
create or replace view {SCHEMA}cube_adjust_daily as
with date_range as (
...
event_day, event_day / 7 as event_week, event_day / 30 as event_month,
...
dateadd(day, event_day, daily_cohort) as effective_date, 
...
from (
  ...
  cohort as daily_cohort, 0 as event_day, cohort_size as cohort_events, 0 as cohort_revenue, cohort_size as time_series_events, 0 as time_series_revenue,
  ...
  where day >= (select start_date from date_range) and day <= (select end_date from date_range) and day = cohort
  union all
  ...
  cohort as daily_cohort, datediff(day, cohort, day) as event_day, events as cohort_events, revenue as cohort_revenue, 0 as time_series_events, 0 as time_series_revenue,
  converted_users, 0 as first_events,
  crystallized as mmp_crystallized
  from {SCHEMA}fetch_adjust_cohorts_measures_daily
  where day >= (select start_date from date_range) and day <= (select end_date from date_range)
  union all
  select {SCHEMA}adjust_network_channel(network) as channel, campaign_id, 
  adgroup_id as adset_id, creative_id as ad_id, os_name as platform, event_name, country, region,
  "date" as daily_cohort, 0 as event_day, 0  as cohort_events, 0 as cohort_revenue, 
...
group by channel, campaign_id, adset_id, ad_id, platform, event_name, daily_cohort, event_day, dynamic_segments
