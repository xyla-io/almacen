...
-- view
--
create or replace view {SCHEMA}cube_adjust_weekly as
with date_range as (
...
null::int as event_day, event_week, event_week * 7 / 30 as event_month,
...
dateadd(day, event_week * 7, daily_cohort) as effective_date,
...
from (
  ...
  cohort as daily_cohort, 0 as event_week, cohort_size as cohort_events, 0 as cohort_revenue, cohort_size as time_series_events, 0 as time_series_revenue,
  cohort_size as converted_users, 0 as first_events,
  crystallized as mmp_crystallized
  from {SCHEMA}fetch_adjust_cohorts_measures_weekly
  where week >= (select start_date from date_range) and week <= (select end_date from date_range) and week = cohort
  union all
  ...
  cohort as daily_cohort, datediff(day, cohort, week) / 7 as event_week, events as cohort_events, revenue as cohort_revenue, 0 as time_series_events, 0 as time_series_revenue,
  converted_users, 0 as first_events,
  crystallized as mmp_crystallized
  from {SCHEMA}fetch_adjust_cohorts_measures_weekly
  where week >= (select start_date from date_range) and week <= (select end_date from date_range)
  union all
  select {SCHEMA}adjust_network_channel(network) as channel, campaign_id, 
  adgroup_id as adset_id, creative_id as ad_id, os_name as platform, event_name, country, region, "date" as daily_cohort,
  0 as event_week, 0  as cohort_events, 0 as cohort_revenue, 
...
group by channel, campaign_id, adset_id, ad_id, platform, event_name, daily_cohort, event_week, dynamic_segments
