...
-- view
--
create or replace view {SCHEMA}cube_apple_search_ads_keyword as
with date_range as (
...
select app_display_name as product_name, campaignid as campaign_id, adgroupid AS adset_id, ad_id, platform, "date" as effective_date, sum(localspend) as spend, sum(impressions) as impressions, sum(taps) as clicks, max(campaignname) as campaign_name, max(adgroupname) as adset_name, max(keyword) as ad_name, MAX(keywordstatus) as ad_status, min(crystallized::int)::bool as crystallized
from {SCHEMA}fetch_apple_search_ads_keywords
